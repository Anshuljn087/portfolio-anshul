    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold sm:text-3xl">Manage experience</h2>
        </div>
        <Link
      ...
      <div className="grid gap-4">
        {experiences.length > 0 ? (
          experiences.map((experience) => (
            <article key={experience.id} className="rounded-[2rem] border border-white/10 bg-white/5 p-5 sm:p-6">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-cyan-300">
                    {experience.role} · {experience.featured ? 'Featured' : 'Normal'}
                  </p>
                  <h3 className="mt-3 text-xl font-semibold sm:text-2xl">{experience.company}</h3>
                  <p className="mt-2 text-sm text-muted-foreground">{experience.summary}</p>
                  <p className="mt-3 text-sm text-muted-foreground">
                    {experience.location ?? 'Remote'} · {experience.startDate.toLocaleDateString()} -{' '}
